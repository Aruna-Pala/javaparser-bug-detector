...
import javaparserBugDetector.BugPattern.InadequateLogInfoInCatchBlockBugPattern;
import javaparserBugDetector.Interfaces.IChecker;

public class InadequateLogInfoInCatchBlockChecker implements IChecker {
                                        ...
                                        for (Node nodeOfOneBlockStatementOfEachCatchBlock : listNodeOfOneBlockStatementOfEachCatchBlock) {
                                            ...
                                            if (nodeOfOneBlockStatementOfEachCatchBlock instanceof MethodCallExpr) {
                                                ...
                                                if (methodName.matches("warn|println|info|debug|error")) {
                                                    ...
                                                    if (shouldReportBug) {
                                                        int lineNumber = CommonUtil.getLineNumber(method);

                                                        // Get the method name by going back up
                                                        String functionName = CommonUtil.getFunctionName(n);

                                                        bugPatterns.add(
                                                                new InadequateLogInfoInCatchBlockBugPattern(lineNumber, file, functionName));
                                                    }
                                                }
                                            }
                                        }
